...
class CSVExperiment:
    ...
    def add_video(self, tag, vid_tensor, global_step: Optional[int] = None, **kwargs):
        dpi = 100
        height, width = vid_tensor.shape[3:]  # Extract height and width from the tensor
        fig_size = (width / dpi * 4, height / dpi * 4)  # Convert to inches
        fig = plt.figure(figsize=fig_size, dpi=dpi, frameon=True)
        ax = plt.Axes(fig, [0., 0., 1., 1.])
        ax.set_axis_off()
        fig.add_axes(ax)
        img_plt = ax.imshow(vid_tensor[0, 0].permute(1, 2, 0), interpolation='none')

        filepath = os.path.join(
            self.log_dir, "videos", "_".join([tag, str(global_step)]) + ".mp4"
        )

        def animate(i):
            x = vid_tensor[0, i].permute(1, 2, 0)
            img_plt.set_data(x)

        myAnimation = animation.FuncAnimation(fig, animate, frames=vid_tensor.shape[1], interval=333, blit=False, repeat=False)
        ...
        myAnimation.save(filepath, writer=FFwriter)
        plt.close(fig)
    ...
    #         torchvision.io.write_video(filepath, vid_tensor, video_codec='MJPEG', fps=self.video_fps)
    #     else:
    #         raise ValueError(
    #             f"Unknown video format {self.video_format}. Must be one of 'pt', 'memmap' or 'mp4'."
    #         )
